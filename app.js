class AudioVisualizer {
    ...
    createParticles() {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            // Brand colors
            colors[i * 3] = 0.12 + Math.random() * 0.3;
    ...
    setupLighting() {
        ...
        // Main point light
        const pointLight = new THREE.PointLight(0x1FB8CD, 1.5, 100);
        pointLight.position.set(0, 5, 5);
        pointLight.castShadow = true;
        this.scene.add(pointLight);

        // Additional colored lights
        const light1 = new THREE.PointLight(0xFF5959, 0.8, 50);
        light1.position.set(-5, 0, 0);
        this.scene.add(light1);

        const light2 = new THREE.PointLight(0x59FF59, 0.8, 50);
        light2.position.set(5, 0, 0);
        this.scene.add(light2);

        const light3 = new THREE.PointLight(0x5959FF, 0.8, 50);
    ...
    updateFrequencyVisualization() {
        ...
        // Update material color based on average frequency
        const avgFreq = this.dataArray.reduce((a, b) => a + b) / this.dataArray.length;
        const hue = (avgFreq / 255) * 0.3 + 0.5; // Keep colors in teal range
    ...
    updateParticleVisualization() {
        ...
        for (let i = 0; i < colors.count; i++) {
            ...
            // Update particle colors based on frequency
            colors.array[i3] = 0.12 + amplitude * 0.5;
            colors.array[i3 + 1] = 0.72 + amplitude * 0.2;
            colors.array[i3 + 2] = 0.80 + amplitude * 0.1;
        }
